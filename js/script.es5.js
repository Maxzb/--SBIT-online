...
/* Переносим меню для мобильных в другое место для правильной работы absolute */

window.onresize = function (event) {
  if (window.innerWidth <= 768) {
    $('nav').insertBefore($('.header'));
  } else {
    $('nav').insertBefore($('.skew'));
  }
};
/* Одинаковая высота для листа в секциях  */


$('.section__list').css('height', '').equalHeights();
/* $('.section__title').css('height', '').equalHeights(); */
...
$('.menu__list').on("click", "a", function (e) {
  ...
  e.preventDefault(); //забираем идентификатор с атрибута href

  var id = $(this).attr('href'),
      //узнаем высоту от начала страницы до блока на который ссылается якорь
  top = $(id).position().top; //анимируем переход на расстояние
